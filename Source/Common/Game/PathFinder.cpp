...
{
    //safety check the state is idle, if it isn't then a path is either already found and in the process of being found
    if(m_State != StateIdle)
    {
        return;
    }
    //cycle through and reset akk the tile path flags to false
...
void PathFinder::update(double aDelta)
{
    ...
    while(isSearchingPath() == true && m_DestinationTileIndex != -1)
    {
        ...
        //Get the node with the lowest F score from the open list, since this is sorted, it should be the first element in the open list
        ...
        if(currentNodeTileIndex == m_DestinationTileIndex)
        {
            ...
            //to track back through the path nodes all the way back ti the start
            ...
            return;
            //if it is we can stop searching and build our final path
        }
        
        //If we got here that means we haven't reach our destination.
        //We need to get the adjacent tiles from the node and add them to the open list
        ...
        for(int i =0; i < adjacentTiles.size(); i++)
        {
            ...
            if(doesTileExistInOpenList(adjacentTile) == false)
            {
                ...
                //If it doesn't create a new path node for the tile
            ...
            else
            {
                ...
                //keep the one with the lower F score, which really means
                //keep the one with lower G score, since the H score
                //will be identical
                PathNode* existingNode = getOpenPathNodeForTile(adjacentTile);
                
                
                
                    //If the tile has a lower F score, 
...
void PathFinder::addAdjacentTile(std::vector<Tile*>& aAdjacentTiles, Tile* aCurrentTile, int aDeltaX, int aDeltaY)
{
    ...
    {
        //Is the tile walkable and valid, (not outside of the bounds of level)
